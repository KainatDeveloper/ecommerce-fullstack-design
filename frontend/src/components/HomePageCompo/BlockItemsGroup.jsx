...
export default function BlockItemsGroup({ interiorProducts }) {
  return (
    <div className="w-full max-w-[1180px] h-auto p-0 gap-0 border border-gray-300 rounded mt-4 bg-base-100 flex flex-col md:flex-row">
      ...
      <div className="max-w-[280px] md:min-h-[380px] overflow-hidden flex relative bg-gradient-to-br from-orange-100 to-orange-50">
        <img
        ...
        <div className="w-full md:absolute z-10 md:p-5 space-y-4 mt-4 md:mt-0 px-4 md:px-0">
          <div>
            <h1 className="text-[24px] md:text-[28px] font-bold text-black leading-tight">
              Home and
            </h1>
            <h1 className="text-[24px] md:text-[28px] font-bold text-black leading-tight">
              outdoor
      ...
      <div className="w-full flex-1 p-4 md:p-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
        {interiorProducts && interiorProducts.length > 0 ? (
